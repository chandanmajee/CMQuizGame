...
function generateQuestion() { let a = rand(1, 10), b = rand(1, 10), c = rand(1, 10), d = rand(1, 10);
...
function checkAnswer(button) { const userAnswer = parseFloat(button.innerText); if (userAnswer === correctAnswer) { score++; document.getElementById("score").innerText = Score: ${score}; const storageKey = highScore_${difficulty}_${currentUsername}; if (score > highScore) { highScore = score; localStorage.setItem(storageKey, highScore); } } }
